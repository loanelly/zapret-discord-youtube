...
## 🔍 Расшифровка частых ошибок в консоли

Если при запуске скриптов или тестов утилита `winws.exe` закрывается, выдает ошибки или сайты по-прежнему не открываются, найдите описание вашей проблемы ниже:

### 🟥 Ошибки сетевого драйвера WinDivert

*   **`WinDivert open error: access denied (код 5)`**
    *   **Причина:** У скрипта нет прав на управление сетевым трафиком системы.
    *   **Решение:** Закройте окно. Нажмите на `.bat` файл правой кнопкой мыши и выберите **«Запуск от имени Администратора»**.

*   **`WinDivert open error: The system cannot find the file specified (код 2)`**
    *   **Причина:** Защитник Windows (Windows Defender) или сторонний антивирус молча заблокировал или удалил файл драйвера `WinDivert64.sys` или `WinDivert.dll`.
    *   **Решение:** Перейдите в настройки антивируса/Защитника, откройте «Журнал защиты», восстановите удаленные файлы проекта и добавьте всю папку скрипта в **Исключения**.

*   **`WinDivert open error: The handle is invalid (код 6)` или зависание на старте**
    *   **Причина:** Конфликт драйверов. В системе одновременно запущен другой обходчик блокировок, который уже захватил управление сетью.
    *   **Решение:** Полностью закройте/удалите GoodbyeDPI, ByeDPI, Spoiler, Amnezia или сторонние VPN. Запустите `service_remove.bat`, перезагрузите ПК и попробуйте снова.

---

### 🟨 Ошибки регистрации служб Windows

*   **`CreateServiceA failed! Error code: 1073 (The specified service already exists)`**
    *   **Причина:** В операционной системе уже зарегистрирована служба с именем `winws1` (например, от старой версии этого проекта).
    *   **Решение:** Запустите файл `service_remove.bat` от имени администратора, чтобы полностью стереть старую службу, после чего повторите установку.

*   **`StartService failed! Error code: 1058`**
    *   **Причина:** Служба `winws1` была отключена в системных настройках Windows вручную.
    *   **Решение:** Нажмите `Win + R`, введите `services.msc`, найдите в списке службу `winws1` (или Zapret), нажмите правой кнопкой мыши -> Свойства. Измените тип запуска на **«Вручную»** или **«Автоматически»**, после чего запустите скрипт заново.

---

### 🟦 Ошибки сети (Служба работает, но сайты не открываются)

*   **Бесконечная загрузка видео, а в консоли пусто**
    *   **Причина:** Текущая стратегия (набор флагов) больше не поддерживается вашим провайдером, либо мешают внутренние настройки браузера.
    *   **Решение:** 
        1. Отключите протокол **Kyber** в браузере (вставьте в адресную строку `chrome://flags/#enable-tls13-kyber` и переведите в **Disabled**).
        2. Отключите поддержку **QUIC** в браузере (`chrome://flags/#enable-quic` -> **Disabled**).
        3. Запустите повторный тест стратегий через встроенное меню `service.bat` (`Run Tests`).
